/*
    ...
    3. It uses individual allocated fixed size array, with additional bookeeping, meaning index base access
        to deque must perform two pointer dereference, but in vector we get in one dereference.
    4. The storeage of a deque is automatically expanded and contracted as needed.
    ...
    6. A deque holding just one element has to allocate its full internal array (e.g. 8 times the object size on)
        64-bit libstdc++;   16 times the object size or 4096 bytes, whichever is larger,
        on a 64-bit libc++).

    TIME COMPLEXITY:
    Random access - contant O(1)
